...
void sendMessage() {  // Function for mesh broadcast
  collisionDetected = true;  // Set the flag when collision is detected
  mesh.sendBroadcast("Collision detected\n");
}
...
void loop() {
  ...
  if (fabs(ax_g) > broadcastThreshold || fabs(ay_g) > broadcastThreshold) {
    sendMessage();  // Trigger the mesh broadcast
  }
...
void handleRoot() {
  ...
  if (collisionDetected) {
    html += "<div style='color: red; font-size: 1.5rem; margin: 20px;'>Collision Detected! Please stop and reset.</div>";
  }
